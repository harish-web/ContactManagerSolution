...
namespace ContacManager.Core.Services
{
    public class PersonsDeleterService : IPersonsDeleterService
    {
        ...
        public PersonsDeleterService(IPersonsRepository personsRepository, ICountriesAdderService countriesService)
        {
        ...
        {
            //Person? selectedPerson =  personsRepository.Persons.Where(e => e.PersonId == personID).Select(e => e).FirstOrDefault(); // Get the first matching person or return null if none exist
            Person? selectedPerson = await personsRepository.GetPersonById(personID);
            if (selectedPerson == null)
            {
                return false;
            }
            //personsRepository.Persons.Remove(selectedPerson);
            //await personsRepository.SaveChangesAsync();
            await personsRepository.DeletePersonByPersonId(personID);
            return true;
        }
